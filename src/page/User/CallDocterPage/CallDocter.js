...
const CallDocter = () => {
  ...
  useEffect(() => {
    const getListCallDoctor = async () => {
      const res = await apiClient.get(`/api/v1/calendars?id=${id}&status=DaXacNhan`);
  ...
  const connectStringee = async (currentUserId) => {
    try {
      ...
      client.on("incomingcall", async (call) => {
        ...
        try {
          const doctorId = String(call.fromNumber || call.callerId);

          const doctor = callDocterData.find(
            (item) => item.bacSi.maBacSi === doctorId
          );

          if (doctor) {
            setIncomingName(doctor.bacSi.taiKhoan.hoVaTen);
          } else {
            setIncomingName("Bác sĩ");
          }
        } catch (err) {
          console.log(err);
          setIncomingName("Bác sĩ");
  ...
  const handleCallDoctor = async (item) => {
    ...
    setCallingName(item.bacSi.taiKhoan.hoVaTen);
              ...
              >
                <div className="wrapper-activiti-calldocter">
                  <img
                    ...
                    src={item.bacSi.avt}
                    alt={item.bacSi.maBacSi}
                  />
                  <p
                    className={item.activiti ? "activiti-calldocter" : ""}
                  ></p>
                </div>

                <div className="infor-calldocter">
                  <div className="name-hour-calldocter">
                    <p className="name-calldocter">{item.bacSi.taiKhoan.hoVaTen}</p>
                  </div>

                  <p className="clinic-calldocter">{item.bacSi.phongKham.tenPhongKham}</p>
                  <p className="hour-calldocter">{item.gioKham}</p>
          ...
          >
            ...
            {callState === "calling" && (
              <div className="popupStyle">
                <p><span>Đang gọi bác sĩ</span> {callingName}...</p>
            ...
            {callState === "ringing" && (
              <div className="popupStyle">
                <h3 style={{ marginLeft: "10px" }}>
                  Bác sĩ {incomingName} đang gọi cho bạn...
                </h3>
